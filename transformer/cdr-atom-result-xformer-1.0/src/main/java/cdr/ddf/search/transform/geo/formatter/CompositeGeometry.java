 ...
 * 
 **/
package cdr.ddf.search.transform.geo.formatter;
...
public abstract class CompositeGeometry {
     ...
     * 
     * @param type
     *            case-sensitive String of the geometric type, must match the exact spelling in the GeoJSON standard
     *            (geojson.org)
     * @param jsonObject
     *            Map of coordinates or geometries
     * @return {@link CompositeGeometry} based on the <code>type</code> passed in, returns <code>null</code> if the type
     *         is not one of the standard GeoJSON geometric strings.
     */
     ...
     *            requires x and y coordinate information as a {@link List}, x coordinate is the first item in the list
    ...
    /**
     * This method is to retrieve numerical information when it is not known how that number will be provided
     * beforehand.
     * 
     * @param object
     * @return double
     ...
     *            a List of [x,y] coordinates. The [x,y] coordinates is a list of two objects where x coordinate is
     *            first and the y coordinate is second in the list
     * @return an array of {@link Coordinate} objects, if input is <code>null</code> or empty, it will return an empty
     *         array
     */
