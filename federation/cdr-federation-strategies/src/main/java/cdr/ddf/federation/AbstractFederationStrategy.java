...
public abstract class AbstractFederationStrategy implements FederationStrategy {
    ...
    /**
     * The {@link List} of pre-federated query plugins to execute on the query request before the query is executed on
     * the {@link Source}.
     */
    private List<PreFederatedQueryPlugin> preQuery;

    /**
     * The {@link List} of post-federated query plugins to execute on the query request after the query is executed on
     * the {@link Source}.
     */
    ...
    private Query getModifiedQuery( Query originalQuery, int numberOfSources, int offset, int pageSize ) {
        ...
        if ( offset > 1 && numberOfSources > 1 ) {
            ...
            if ( LOGGER.isDebugEnabled() ) {
                LOGGER.debug( "Creating new query for federated sources to query each source from " + modifiedOffset + " to " + modifiedPageSize + "." );
            ...
            /**
             * Federated sources always query from offset of 1. When all query results are received from all federated
             * sources and merged together - then the offset is applied.
             * 
             */
    ...
    private class CallableSourceResponse implements Callable<SourceResponse> {
        ...
        public SourceResponse call() throws Exception {
            ...
            LOGGER.debug( "The source {} responded to the query in {} milliseconds", source.getId(), elapsedTime );
            sourceResponse.getProperties().put( SearchConstants.ELAPSED_TIME, elapsedTime );
