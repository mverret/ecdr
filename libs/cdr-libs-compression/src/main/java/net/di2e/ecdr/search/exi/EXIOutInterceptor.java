 ...
 *
 **/
package net.di2e.ecdr.search.exi;
...
public class EXIOutInterceptor extends AbstractPhaseInterceptor<Message> {
    ...
    public void handleMessage(Message message) {
        ...
        if (isRequestor(message)) {
            ...
            //TODO add support for client sending request
            LOGGER.debug("Sending request encoded using EXI.");
        } else {
            ...
            if (requestHeaders != null) {
                String acceptEncodingHeader = StringUtils.join(requestHeaders.get(HttpHeaders.ACCEPT_ENCODING), ",");
                if (StringUtils.isNotBlank(acceptEncodingHeader) && acceptEncodingHeader.contains("x-exi")) {
                ...
                LOGGER.debug("No request headers were found in the incoming request. Cannot encode to exi.");
            }
        }
    }
    ...
    private class EXIOutputStream extends CachedOutputStream {
        ...
        protected void doClose() throws IOException {
            ...
            try {
                ...
                LOGGER.warn("Encountered exception when trying to encode outgoing response into EXI. Sending back uncompressed response.", exp);
